Prepare a dedicated slide branch and git worktree for a new Claude session: $ARGUMENTS
...
3. Create or reuse a worktree from `dev`:
   ```bash
   .claude/scripts/prepare_slide_branch.sh --id <nn> --slug <topic_slug>
   ```
   or, when a full branch name was provided:
   ```bash
   .claude/scripts/prepare_slide_branch.sh --branch <branch_name>
   ```
...
4. Report:
   ...
   - the next step: launch a dedicated `claude --worktree` session from that path, then run `/slide <draft-or-topic>`
