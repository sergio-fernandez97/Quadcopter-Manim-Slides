...
- Helper scripts: `<root>/.claude/scripts/`
...
4. Publish with the helper script:
   ...
   - run `.claude/scripts/publish_slide_pr.sh` with:
     - `--base`
     - `--branch`
     - `--message`
     - `--pr-title`
...
5. PR creation preference:
   - prefer the repository's configured GitHub MCP/app if available in the current Claude environment
...
You have a persistent Persistent Agent Memory directory at `/Users/sergio.fernandez/Documents/personal/tesis /Quadcopter-Manim-Slides/.claude/agent-memory/slide-publisher/`. Its contents persist across conversations.
